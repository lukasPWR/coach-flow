...
<template>
  ...
  >
    <CardHeader class="flex flex-row items-start justify-between space-y-0 pb-2 gap-2">
      ...
      <Badge :variant="plan.status === PlanStatus.ACTIVE ? 'default' : 'secondary'" class="shrink-0">
        {{ plan.status }}
      </Badge>
    </CardHeader>
    <CardContent class="flex-grow">
      <div class="flex items-center space-x-2 text-sm text-muted-foreground mt-2">
        <Avatar class="h-6 w-6">
          <AvatarImage :src="plan.clientAvatar" v-if="plan.clientAvatar" />
          <AvatarFallback class="text-[10px]">{{ clientInitials }}</AvatarFallback>
        </Avatar>
