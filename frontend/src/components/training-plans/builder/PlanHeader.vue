...
const handleStatusChange = () => {
  const newStatus = props.plan.status === PlanStatus.ACTIVE ? PlanStatus.ARCHIVED : PlanStatus.ACTIVE;
...
<template>
  <div class="flex items-center justify-between gap-4 p-4 border-b bg-card">
    <div class="flex items-center gap-4 flex-1">
      ...
      <div class="w-[200px]">
          <Select :model-value="plan.clientId || undefined" @update:model-value="handleClientChange">
            <SelectTrigger>
    ...
    <div class="flex items-center gap-3">
        <Badge :variant="plan.status === PlanStatus.ACTIVE ? 'default' : 'secondary'" class="cursor-pointer" @click="handleStatusChange">
