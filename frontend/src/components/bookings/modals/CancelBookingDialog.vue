...
@update:open="$emit('update:open', $event)">
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription> Czy na pewno chcesz anulować rezerwację? </DialogDescription>
      </DialogHeader>

      <div
        v-if="isLateCancellation"
        class="bg-destructive/10 p-4 rounded-md flex items-start space-x-2 text-destructive"
      >
...
const emit = defineEmits<{
  ...
  confirm: [booking: BookingViewModel];
}>();
